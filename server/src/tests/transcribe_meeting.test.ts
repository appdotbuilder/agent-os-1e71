...
const smallAudioData = Buffer.from('small audio content for testing').toString('base64'); // ~33 bytes -> < 10KB
const mediumAudioData = Buffer.from('medium sized audio content for testing purposes - this is longer content to simulate a medium audio chunk that would generate partial transcripts'.repeat(100)).toString('base64'); // ~15KB -> between 10-50KB
const largeAudioData = Buffer.from('large audio content for testing purposes - this represents a complete audio recording that should generate full transcripts'.repeat(500)).toString('base64'); // ~60KB -> > 50KB
...
describe('transcribeMeeting', () => {
    ...
    it('should transcribe large audio and return complete transcript', async () => {
        ...
        expect(result.partial_transcript.length).toBeGreaterThan(100);
    });
    ...
    it('should throw error for invalid base64 audio data', async () => {
        const input: TranscribeMeetingInput = {
            workspace_id: 1,
            audio_data: 'this-is-not-valid-base64!@#$%^&*()'
        };

        await expect(transcribeMeeting(input)).rejects.toThrow(/invalid base64/i);
    });
