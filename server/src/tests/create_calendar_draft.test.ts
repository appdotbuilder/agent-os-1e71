...
describe('createCalendarDraft', () => {
    ...
    it('should create a calendar draft with all required fields', async () => {
        ...
        // Verify input data (dates get serialized as ISO strings in JSON)
        const storedInput = result.draft_event.input as CreateCalendarDraftInput;
        expect(storedInput.workspace_id).toEqual(testInput.workspace_id);
        expect(storedInput.title).toEqual(testInput.title);
        if (testInput.description !== undefined) {
            expect(storedInput.description).toEqual(testInput.description);
        }
        expect(new Date(storedInput.start_time)).toEqual(testInput.start_time);
        expect(new Date(storedInput.end_time)).toEqual(testInput.end_time);
        if (testInput.attendees !== undefined) {
    ...
    it('should save agent event to database', async () => {
        ...
        // Verify input data (dates get serialized as ISO strings in JSON)
        const savedInput = savedEvent.input as CreateCalendarDraftInput;
        expect(savedInput.workspace_id).toEqual(testInput.workspace_id);
        expect(savedInput.title).toEqual(testInput.title);
        expect(new Date(savedInput.start_time)).toEqual(testInput.start_time);
        expect(new Date(savedInput.end_time)).toEqual(testInput.end_time);
    ...
    it('should handle complex calendar events with multiple attendees', async () => {
        ...
        expect((result.draft_event.input as CreateCalendarDraftInput).attendees).toHaveLength(5);
    });
