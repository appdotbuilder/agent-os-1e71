...
const extractEntities = (transcript: string): Record<string, unknown> => {
    ...
    // Extract dates (more flexible pattern matching)
    ...
    entities['decisions'] = sentences.filter((sentence: string) => 
...
export const finalizeMeeting = async (input: FinalizeMeetingInput): Promise<MeetingFinalizationResponse> => {
    try {
        ...
        const peopleArray = entities['people'] as string[];
        const decisionsArray = entities['decisions'] as string[];
...
## Key People
${peopleArray.length > 0 ? peopleArray.join(', ') : 'None identified'}

## Decisions Made
${decisionsArray.length > 0 ? decisionsArray.map((d: string) => `- ${d}`).join('\n') : 'None identified'}
